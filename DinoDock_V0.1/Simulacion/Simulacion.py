...
def ingresar_contenedor(marca,funcion=Funcion_Opti.poner_contenedor):
    global bloque
    coordenadas = funcion(marca, bloque)
...
def sacar_contenedor(marca, funcion=Funcion_Opti.sacar_contenedor):
    global bloque
    ...
    with open(ruta, "r") as archivo: 
        for i in archivo:
            ...
            if fecha_actual != datetime.strptime(data[2], '%d-%m-%Y %H:%M').date():
                ...
                fecha_actual = datetime.strptime(data[2], '%d-%m-%Y %H:%M')
                bloque.fecha = fecha_actual
